...
public class AssignServiceTask extends TimerTask {
     ...
     * Assignments is returned. The order in which services are assigned are
     * defined by the company's sorting behavior. The new Assignment List is
     * then added to the ServiceAssigment Registry
     */
    @Override
    public void run() {
        ...
        // returns the sorting behavior for the assignments
        ISortingBehavior sorting = company.getServiceSortingBehavior();

        // returns the list of assignments from the assignment algoritm
        IAssignmentAlgoritm alg = company.getAssignmentAlgoritm();
        List<ServiceAssignment> sal = alg.assignServices(unassignedServices, sorting);
